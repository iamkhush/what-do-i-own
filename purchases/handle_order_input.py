...
from .models import PurchaseOrder, PurchaseLineItem, Order
from stores.models import Store
from purchase.models import Purchase
from purchasers.models import Purchaser

def handle_order_input(order: Order) -> None:
    store, _ = Store.objects.get_or_create(name__iexact=order.store, defaults={'name': order.store})
    ...
    for purchase in order.purchases:
        item, _ = Purchase.objects.get_or_create(name__iexact=purchase.name, defaults={'name': purchase.name})
        actual_unit = 1
