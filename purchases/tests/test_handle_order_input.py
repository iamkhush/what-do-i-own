...
from purchases.models import PurchaseOrder, PurchaseLineItem, Order, PurchaseModel
...
class HandleOrderInputTest(TestCase):
    ...
    def test_handle_order_input_creates_database_rows(self):
        sample_order = Order(
            ...
            store="Test Store",
            purchases=[
                PurchaseModel(name="Test Product 1", quantity=2, quantity_unit="PIECE", price=50),
                PurchaseModel(name="Test Product 2", quantity=500, quantity_unit="GRAMS", price=25),
            ]
        )
        ...
        self.assertEqual(purchase_order.purchase_date, datetime.strptime("2023-10-01", '%Y-%m-%d').date())    
        ...
        purchase_line_item_1 = purchase_line_items.get(purchase__name__iexact="Test Product 1")
        ...
        purchase_line_item_2 = purchase_line_items.get(purchase__name__iexact="Test Product 2")
